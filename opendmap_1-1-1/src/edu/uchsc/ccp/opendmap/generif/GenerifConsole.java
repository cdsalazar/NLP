 ...
 * September 2005
 */
package edu.uchsc.ccp.opendmap.generif;
...
import edu.uchsc.ccp.opendmap.Translator;
/**
...
public class GenerifConsole {
	...
	private static void writeOutput(DMAPItem item, Reference ref, int indent, String prefix) {
		if (prefix != null) {
			...
			writeLine(indent, item.getText() + " (" + start + "-" + end + "-" + ref.getMissing() + ") " 
	...
	public static void main(String[] args) throws ParseException{
		...
		System.out.println();
		System.out.println("Welcome, please enter NL to be translated:");
		System.out.println("When you are finished, enter: quit, done, stop, bye, or goodbye.");
		...
		while (!done) {
			...
			if (!done) {
				...
				Collection<Reference> responses = GenerifProcessor.processUtterance(p, utterance, conceptsOfInterest);
				...
				if (responses.isEmpty()) {
					...
					Translator t = new Translator(utterance, responses);
